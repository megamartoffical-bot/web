...
export const vendorTotalOrdersStats = async (
  days: number = 7,
  vendorId?: string
) => {
...
export const vendorTotalProfitStats = async (
  days: number = 7,
  vendorId?: string
) => {
  const endDate = new Date();
...
export const VendorTopSellingProducts = async (vendorId?: string) => {
...
export const getWithdrawalStats = async (vendorId: string) => {
  const objectId = new Types.ObjectId(vendorId);

  const withdrawals = await WithdrawalsModel.aggregate([
  ...
  const requestedBalance = withdrawals[0]?.requestedBalance || 0;
  const withdrawnBalance = withdrawals[0]?.withdrawnBalance || 0;
