import { getWithdrawalStats, vendorMonthlySalesHistory, vendorOrderCountByStatus, vendorRecentOrders, vendorSalesAndCostStats, vendorTodayOrdersStats, VendorTopSellingProducts, vendorTotalOrdersStats, vendorTotalProfitStats, vendorTotalShopsStats, vendorTrendingProductsStats } from "./vendor.utils";
...
export const getVendorStatsFromDB = async (payload: IAdminStatsPropsValues, vendorId: string) => {
  ...
  const WithdrawalStats = await getWithdrawalStats(vendorId);
  return {
    SalesAndCostStats,
    TotalOrdersStats,
    ProfitStats,
