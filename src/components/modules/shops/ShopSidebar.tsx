...
export function ShopSidebar() {
  ...
  const shopProducts: Product[] = allProducts.filter(
    (p: Product) => String(p.shopId) === String(shopId)
  );
  const navLinks = [
    { href: `/shops/shop/${shopId}`, label: "Shop", icon: Home },
    { href: `/shops/shop/${shopId}/offers`, label: "Offers", icon: Gift },
    { href: `/shops/contact/${shopId}`, label: "Contact", icon: Phone },
    { href: "/#", label: "Website", icon: Globe },
  ];

  const infoLinks = [
    {
      href: `/shops/terms/${shopId}`,
    ...
    <Card className="w-full p-4 flex flex-col gap-4 h-fit">
      ...
      <div className="flex flex-col items-center text-center">
        ...
        <p className="text-sm text-[#64748B] mt-2">
          {shopData?.data?.basicInfo?.description}
        </p>
      </div>
      ...
      <div className="text-sm space-y-4">
        ...
        <div className="flex gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-[#0F172A]">
              {`${shopData?.data?.shopAddress?.streetAddress || ""}, ${
                shopData?.data?.shopAddress?.city || ""
              }, ${shopData?.data?.shopAddress?.state || ""}, ${
                shopData?.data?.shopAddress?.zip || ""
              }, ${shopData?.data?.shopAddress?.country || ""}`}
            </p>
          </div>
        </div>
        ...
        <div className="flex gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-[#0F172A]">
              {shopData?.data?.shopSetting?.contactNo}
            </p>
          </div>
        </div>
      </div>
