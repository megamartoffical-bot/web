...
import { useUpdateCustomerMutation, useGetSingleCustomerQuery } from "@/redux/featured/customer/customerApi";
...
export default function AddAddressForm() {
  ...
  // fetch current customer (skip if no user)
  const { data: singleCustomer } = useGetSingleCustomerQuery(
    currentUser?._id ?? "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!singleCustomer?._id) {
      return;
    }

    const newAddress = {
      type: "shipping",
      title: formData.addressName || formData.addressType,
  ...
  return (
    <Card className="w-full rounded-lg p-6 border">
      <CardHeader>
